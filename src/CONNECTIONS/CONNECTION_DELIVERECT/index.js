...
async function eventHookLogic (config, connectionContainer) {
  ...
  const [channelLinkId,,, apiEndpoint] = config
  ...
  try {
    ...
    const r = await makeRequest(
      token,
      'post',
      `${apiEndpoint}/${CHANNEL_NAME}/order/${channelLinkId}`,
      body
    )
...
module.exports = new Connection({
  ...
  configNames: ['Channel link ID', '"Not busy" flow ID', '"Busy" flow ID', 'API endpoint', 'API client ID', 'API client secret'],
  configDefaults: ['', '', '', 'https://api.staging.deliverect.com', '', ''],
  methods: {
