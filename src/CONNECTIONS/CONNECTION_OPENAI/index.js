...
const { default: PROMPT_CHAT } = require('./PROMPT_CHAT')
    ...
    'MESSAGE': async function (config, connectionContainer) {
      ...
      try {
        ...
        for (let i = 0; i < chatFederatedUsers.length; i++) {
          ...
          console.warn('[DebugLater70] 2', JSON.stringify({ data1, output }, null, 2))
          ...
          const finalMessage = texts.join('\n\n').trim() || _('MAGIC_BUCKET_NO_REPLY', user.language)

          console.warn('[DebugLater70] 2', { finalMessage })
          ...
          rdic.get('bus').send({
            roomId: `${user.publicKey}---generic`,
            data: {
              applicationBlock: '62fe1627-bad5-4523-a935-ba1e0f03236f',
              applicationId: application.id,
              action: 'refresh'
            }
          })
        ...
        createEvent({
          ...
          customData: { id: 'CONNECTION_OPENAI', message }
        })
      }
    ...
    magic: {
      ...
      logic: async ({ connectionContainer, config, body }) => {
        ...
        return typeof text === 'string'
          ? text
          : _('MAGIC_BUCKET_NO_REPLY', user.language)
